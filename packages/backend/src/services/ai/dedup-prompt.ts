...
- **开头必须换角度**：第一段不能和原文以相同角度切入（如原文从外貌写起则改从评论切入，原文从背景写起则改从结论引入）
- **段落顺序必须调整**：至少有 2 处段落位置与原文不同，不能按原文顺序逐段改写
- **先结论后原因**：原文「先铺垫后结论」的段落，改写时倒过来——先说结论，再补原因

## 自检（输出前必须过一遍）

逐句扫描：是否存在与原文连续 8 字以上相同的片段？如果有，重写那句。
段落顺序是否和原文完全一致？如果是，至少调整 2 处。
